...
$query = mysqli_query($con, "SELECT * FROM product JOIN orders ON orders.pid = product.pid WHERE orders.userid = $_SESSION[userid]");
?>
...
<body>
    ...
    while ($dbrow = mysqli_fetch_assoc($query)) {
        $query2 = mysqli_query($con, "SELECT confirmed FROM orders WHERE orderid = $dbrow[orderid]");
        ...
        <div class='p-2 border-bottom d-inline-block'>
            <div class='card'>
                ...
                <button <?php echo $disabled; ?> class='btn btn-danger' id='cbtn'><?php echo $confirmationText; ?></button>
            </div>
        </div>
    <?php
    }
