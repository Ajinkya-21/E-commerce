...
$query = mysqli_query($con, "SELECT * FROM product JOIN orders ON product.pid = orders.pid");
...
while ($dbrow = mysqli_fetch_assoc($query)) {
    $query2 = mysqli_query($con, "SELECT username, userID FROM user WHERE userId = '{$dbrow['userid']}'");
    $username_row = mysqli_fetch_assoc($query2);
    $username = $username_row['username'];
    $userid = $username_row['userID'];
